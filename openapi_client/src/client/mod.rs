...
impl<F, C> Api<C> for Client<F> where
    ...
    fn get(&self, param_device: String, param_baudrate: models::Baudrate, param_address: i32, context: &C) -> Box<Future<Item=GetResponse, Error=ApiError>> {
        let mut uri = format!(
            ...
            self.base_path, device=utf8_percent_encode(&param_device.to_string(), ID_ENCODE_SET), baudrate=utf8_percent_encode(&param_baudrate.to_string(), ID_ENCODE_SET), address=utf8_percent_encode(&param_address.to_string(), ID_ENCODE_SET)
        );
    ...
    fn scan(&self, param_device: String, param_baudrate: models::Baudrate, context: &C) -> Box<Future<Item=ScanResponse, Error=ApiError>> {
        let mut uri = format!(
            ...
            self.base_path, device=utf8_percent_encode(&param_device.to_string(), ID_ENCODE_SET), baudrate=utf8_percent_encode(&param_baudrate.to_string(), ID_ENCODE_SET)
        );
